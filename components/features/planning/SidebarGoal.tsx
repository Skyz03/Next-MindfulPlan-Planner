...
export default function SidebarGoal({ goal }: { goal: any }) {
    ...
    return (
        <div className="group/goal relative pl-2">
                ...
                <div className="min-w-0 flex-1 py-1">
                    <div className="flex items-center justify-between group/title">
                        ...
                        <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover/title:opacity-100">
                            ...
                            <form action={deleteGoalAction}>
                                ...
                                <button className="rounded p-1 text-stone-300 hover:bg-red-50 hover:text-red-400 dark:hover:bg-red-900/20 transition-colors">
                    ...
                    {!isExpanded && (
                        <div
                            ...
                            className="text-[10px] text-stone-400 cursor-pointer hover:text-stone-500 mt-0.5 truncate"
                        >
            ...
            {isExpanded && (
                <div className="relative space-y-2 pl-9 pr-1 pb-4 animate-in slide-in-from-top-2 duration-200 fade-in">
                    ...
                    {goal.steps.map((task: any) => (
                        <DraggableTask key={task.id} task={task}>
                            <div className="group flex cursor-grab items-center justify-between rounded-lg border border-stone-100 bg-white px-3 py-2.5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md active:cursor-grabbing dark:border-stone-800 dark:bg-[#222] dark:hover:border-orange-900">
                                ...
                                <div className="pl-2 text-stone-300 opacity-0 group-hover:opacity-100">
                                    ⋮⋮
                                </div>
                            </div>
                        </DraggableTask>
                    ...
                    <form
                        action={addTask}
                        className="mt-2 opacity-60 transition-opacity hover:opacity-100 relative group/add"
                    >
                        <input type="hidden" name="date_type" value="backlog" />
                        <input type="hidden" name="goal_id" value={goal.id} />

                        <div className="flex items-center gap-2">
                            <div className="w-4 h-4 rounded flex items-center justify-center bg-stone-100 dark:bg-stone-800 text-stone-400 group-hover/add:bg-orange-100 group-hover/add:text-orange-500 transition-colors">
                            ...
                            <select
                                name="priority"
                                defaultValue="medium"
                                className="rounded border border-stone-200 bg-transparent px-1.5 py-0.5 text-[10px] text-stone-500 outline-none dark:border-stone-700 dark:text-stone-400 opacity-0 focus:opacity-100 group-hover/add:opacity-100 transition-opacity"
                            >
